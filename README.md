...
你可以在 Release 页面下载与系统对应的可执行文件, 通常来说, 作为一个 windows 用户, 你需要下载 mahjim_windows_amd64.exe, 双击运行之后它监听的地址是 `localhost:8080/`, 之后你只要在浏览器里打开这个地址(你可以点击 [这个链接](localhost:8080/) 快速打开), 之后就可以使用了. 如果你使用 markdown 就可以通过输入 `![](localhost:8080/123s)`  来获得一张图片了.
